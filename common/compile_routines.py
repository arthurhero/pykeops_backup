...
def get_or_build_pybind11_template(
    ...
    template_build_folder = os.path.join(
        pykeops.config.bin_folder,
        "build-pybind11_template-"+template_name
    )

    is_rebuilt = False
    if not os.path.exists(os.path.join(template_build_folder,"CMakeCache.txt")):
        is_rebuilt = True
        print(
        ...
        os.makedirs(template_build_folder, exist_ok=True)
        ...
        os.rename(
            os.path.join(pykeops.config.bin_folde,fname),
            os.path.join(template_build_folder,fname)
        )
...
def create_keops_include_file(
    ...
    template_include_file = os.path.join(pykeops.config.script_formula_folder,"formula.h.in")
    target_include_file = os.path.join(
        build_folder, pykeops.config.shared_obj_name + ".h"
    )
...
def get_build_folder_name_and_command(dtype, lang, include_dirs):
    ...
    build_folder = os.path.join(
        pykeops.config.bin_folder,
        "build-" + sha256("".join(command_line).encode("utf-8")).hexdigest()[:10]
    )
...
def compile_generic_routine(
    ...
    print(
        ...
        + dllname
        + " in "
        + os.path.realpath(os.path.join(build_folder,".."))
        + ":\n"
        + "       formula: "
        + formula
        + "\n"
    ...
    if not is_rebuilt:
        fname = pykeops.config.shared_obj_name + ".o"
        os.rename(
            os.path.join(pykeops.config.bin_folder, fname),
            os.path.join(template_build_folder, fname)
        )
        run_and_display(
            ["cmake", "--build", ".", "--target", template_name, "--", "VERBOSE=1"],
    ...
    os.makedirs(os.path.join(pykeops.config.bin_folder, dllname), exist_ok=True)
    print("template_build_folder",template_build_folder)
    ...
    os.rename(
        os.path.join(template_build_folder,fname),
        os.path.join(pykeops.config.bin_folder, dllname, fname)
    )
